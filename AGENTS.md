...
| 🟡 Medio | Phase I.9 — `#[serde(rename_all = "camelCase")]` en DTOs restantes + rename masivo de campos snake→camel en frontend (`InvestigadorDto`, `InvestigadorDetalleDto`, `PublicacionDto`, `ReniecDniLookupResult`, `RenacytLookupResult`, `SyncPublicacionesResult`, `CreateInvestigadorRenacytRequest`). Blast-radius: 9 DTOs Rust, ~12 archivos TS (`src/shared/tauri/types/*`, `src/shared/tauri/investigadores.ts`, `src/shared/tauri/pure.ts`, `src/features/investigadores/hooks/useInvestigadorCreateForm.ts`, `src/features/investigadores/components/{InvestigadorPublicacionesSection,InvestigadoresTableGrid,InvestigadorDetailScreen,detail/InvestigadorDetailRenacytSection}.tsx`, `src/features/proyectos/api.ts`, `src/features/dashboard/types.ts`, `src/features/eventos/api.ts`, `src/features/usuarios/hooks/useUsuariosTab.ts`, `src/features/auth/wizard/useWizardCreateAdmin.ts`). Regla AGENTS.md: TODO `#[tauri::command]` con fields multi-word debe llevar `rename_all = "camelCase"`. El bug actual: frontend envía snake_case (`id_grado`, `apellido_paterno`, `ficha_url`) pero backend espera camelCase — comportamiento silencioso por defecto de serde. |
